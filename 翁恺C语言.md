  ...
  * 结构可以后期整体赋值，而数组只能在初始化的时候整体赋值，后期想赋值只能遍历
    ...
    * ```c
      ...
      int main()
      {
          ...
          p1=(struct space){11,45,14};
          printf("p1.x=%d\tp1.y=%d\tp1.z=%d",p1.x,p1.y,p1.z);
          return 0;
      }
      ```

    * ```
      p1.x=11	p1.y=45	p1.z=14
      ```
...
int main()
{
    ...
    struct space *p=&p1;
    printf("*p=&p1\n");
    //printf("p.x=%d\tp1.y=%p\tp1.z=%p\n",p.x,p.y,p.z);//Error: Member reference type 'struct space *' is a pointer; did you mean to use '->'?      Warning: Format specifies type 'void *' but the argument has type 'int'
    printf("p->x=%d=p1.x\tp->y=%d=p1.y\tp->z=%d=p1.z\n",p->x,p->y,p->z);
    printf("p=%p\t&p->x=%p\t&p->y=%p\t&p->z=%p\n",p,&p->x,&p->y,&p->z);
    printf("&p1=%p\t&p1.x=%p\t&p1.y=%p\t&p1.z=%p\n",&p1,&p1.x,&p1.y,&p1.z);
    printf("既然直觉已经被颠覆，那就贯彻到底咯\n&p=%p\t可以发现结构指针p的地址就比p1地址少8个字节",&p);
    return 0;
}
```
...
```
*p=&p1
p->x=11=p1.x	p->y=45=p1.y	p->z=14=p1.z
p=0x30de486e0	&p->x=0x30de486e0	&p->y=0x30de486e4	&p->z=0x30de486e8
&p1=0x30de486e0	&p1.x=0x30de486e0	&p1.y=0x30de486e4	&p1.z=0x30de486e8
既然直觉已经被颠覆，那就贯彻到底咯
&p=0x30de486d8	可以发现结构指针p的地址就比p1地址少8个字节
```

总而言之，这和之前学的指针不自洽！按直觉，在`struct space *p=&p1;`的情况下，应当有`p==&p1` `p.x==&p1.x` `*p.x=p1.x`

但根据编译器warning可以判断出`p.x`是int类型，并不是一个指向`p1.x`的指针变量。

还要引入新符号`->`搭配结构指针的使用 ,围绕此有`p->x==p1.x` `&p->x==&p1.x`。

> 和此前不自洽，大概是为了实现更重要的功能。

> 不过，`可以发现结构指针p的地址就比p1地址少8个字节`这是和之前研究的结构存储方式自洽：
>
> ```mermaid
> flowchart TB
> 	subgraph 后定义的p2
...
但是`Warning:形参 从未使用`可以说明：`getstruct(today)`能成功读入`today`是因为`today`是全局变量，所以函数里的`scanf("%d%d%d",&today.y,&today.m,&today.d);`操作执行之后，`today`就被读入了。参数并没有起作用，就是走了个过场罢了。
...
是不是因为之前没遇到这种情况（函数的功能是scanf很多个变量）?那就试试看：
...
* 实参在main函数里定义
  ...
  * ```
    ...
    * 发现实参t没有成功读取到输入的值，这是很显然的，因为根本没有用指针，当然无法改变外面的值

      * 之所以这样尝试，是为了与之前的尝试对应

        > 我也是服了当初的尝试了，让我现在真的好难搞。

* 实参是全局变量

  * ```c
    #include <stdio.h>
    int t/*Previous declaration is here*/;
    void getnum(int t/*Warning:Declaration shadows a variable in the global scope*/)
    {
        scanf("%d",&t);
    }
    int main()
  ...
  * ```
    ...
    * 出现了！和之前一样的warning和结果！形参shadow了全局变量！

    * 联系`结构可以作为函数的参数，其原理是：函数内部新生成一个结构，并复制外面的结构的值到函数里面的结构。然后也可以返回一个结构。` 可以知道

      * 参数传递只传递了值！这是最关键的认知！

      * 如果形参和实参重名

        * 即便实参是全局变量，在函数内部也会被形参shadow掉

          > 不过这种情况下为什么`t=0`呢？
          >
          > 那又为什么，实参是main里的变量的时候，t的值很乱呢？

    

我现在需要及时止损！我现在的困惑很大程度上来源于“全局变量”没学。
...
> 过去的奇怪研究，逻辑混乱！在遇到下面情况之后竟然又多用了一个输出函数，而且完全没有意识到自己的形参不是数组
>
> ```
> Please input today:1919 8 10
> today is 0-0-0
